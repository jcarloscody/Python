...
def main(main_images_folder, new_width=800):
    if not os.path.isdir(main_images_folder):  #verificando se o caminho não existe.
        raise NotADirectoryError(f'{main_images_folder} não existe.') #se não existe levanta uma exceção

    for root, dirs, files in os.walk(main_images_folder):
        for file in files:
            file_full_path = os.path.join(root, file)
            file_name, extension = os.path.splitext(file) #separando o arquivo e a extenção
